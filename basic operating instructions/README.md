...
- `MODEL_GRAPH_AND_META_OPTIMIZATION_BOUNDARY.md` — model-graph namespace, learned/meta-optimizer proposal boundaries, anti-homunculus constraints, objective provenance, component version coherence, and self-referential update governance;
- `PROTECTED_UPDATE_ACTIVATION_AND_REQUALIFICATION.md` — commit-time authority revalidation, mixed-version semantic isolation, capability-scoped post-activation inhibition, and governed rollback/forward repair.
...
Machine-readable counterparts and conformance checks live under `../specs/`, especially `HC_COGNITIVE_ORGAN_INVARIANTS_V1.yaml`, `HC_CONFORMANCE_SUITE_V1.yaml`, `HC_MEMORY_PROVIDER_BOUNDARY_V1.yaml`, `HC_BOOTSTRAP_RECOVERY_V1.yaml`, `HC_PROTECTED_UPDATE_GOVERNANCE_V1.yaml`, and `HC_PROTECTED_UPDATE_ACTIVATION_SAFETY_V1.yaml`.
